  ...
  def equivalent(self, cir_arr):
    ...
    # if the sizes are same
      ...
      for i in range(self.size):
        ...
        if (self.cir[index1]==cir_arr.cir[index2]):
            isEqual = True

            #update the index for both array
            index1 = (index1+1)%len(self.cir)
            index2 = (index2+1)%len(cir_arr.cir)

        # if any pair of items mismatch, then break/return since they are not equivalent
        else:
          isEqual = False
          return isEqual
    ...
    for i in range(self.size):
      ...
      for j in range(c2.size):
        ...
        if (cir1[index1]==cir2[index2]):
          temp[index3] = cir1[index1]
          index3+=1
        
        index2 = (index2+1)%len(cir2)
      index1 = (index1+1)%len(cir1)

    # update Final linear array
    linear = [None]*index3
